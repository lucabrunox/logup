...
macro_rules! is_resource_already_exists_exception {
    ($err:expr) => {
        $err.as_service_error().map(|se| se.is_resource_already_exists_exception()).unwrap_or(false)
    }
}

async fn create_log_stream(client: &Client, log_group_name: &str, log_stream_name: &str) {
    let resp = client.create_log_stream()
    ...
    if let Err(e) = resp {
        if !is_resource_already_exists_exception!(e) {
            report_err(&e);
            panic!("Error creating log stream {}:log-stream:{}: {}", log_group_name, log_stream_name, e);
        }
    }
}

async fn create_log_group(client: &Client, log_group_name: &str) {
    let resp = client.create_log_group()
        .log_group_name(log_group_name)
        .send().await;
